...
public class Tank extends src.gameObjects.GameObject implements Collidable {
    ...
    private void Shoot() {
        if (System.currentTimeMillis() - firingTimer > 1000) {
            src.gameObjects.Bullet blt = new src.gameObjects.Bullet(x + 17, y + 17, angle, bulletImg, smallExplosionImg, tankName);
    ...
    public void collision(Collidable obj) {
        ...
        if (obj instanceof src.gameObjects.Bullet && ((src.gameObjects.Bullet) obj).getOwner() != this.tankName) {
            this.depleteHealth();
        }
        if (obj instanceof src.gameObjects.PowerUp && lives < 4) {
            count++;
