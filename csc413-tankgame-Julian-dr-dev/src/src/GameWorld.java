...
public class GameWorld extends JPanel {
    ...
    private final ArrayList <src.gameObjects.GameObject> gameObjects = new ArrayList < > ();

    public void addGameObject(src.gameObjects.GameObject obj) {
    ...
    private void init() {
        ...
        try {
            backgroundImg = ImageIO.read(getClass().getClassLoader().getResource("Background.bmp"));
            tankImg = ImageIO.read(getClass().getClassLoader().getResource("tank1.png"));
            lifeImg = ImageIO.read(getClass().getClassLoader().getResource("life.gif"));
            bulletImg = ImageIO.read(getClass().getClassLoader().getResource("Weapon.gif"));
            imagesmallexplosion = ImageIO.read(getClass().getClassLoader().getResource("Explosion_small.gif"));
            imagelargeexplosion = ImageIO.read(getClass().getClassLoader().getResource("Explosion_large.gif"));
            wallImg = ImageIO.read(getClass().getClassLoader().getResource("Wall2.gif"));
            breakableWallImg = ImageIO.read(getClass().getClassLoader().getResource("Wall1.png"));
    ...
    private void createMap() {
        try {
            BufferedReader br = new BufferedReader(new InputStreamReader(getClass().getClassLoader().getResourceAsStream("Map.txt")));
            String line;
            int j = 0;
                ...
                for (int i = 0; i < tokens.length; i++) {
                    switch (tokens[i]) {
                        ...
                        case "2":
                            ...
                            break;
                        case "3":
                            ta1 = new Tank(i * 32, j * 32, 0, 0, 0, tankImg, bulletImg, "tankerPlayerOne", this);
                            ...
                            break;
                        case "4":
                            tb2 = new Tank(i * 32, j * 32, 0, 0, 0, tankImg, bulletImg, "tankerPlayerTwo", this);
        ...
        for (int i = 0; i < gameObjects.size(); i++) {
            if (gameObjects.get(i).isAlive()) {
                ...
                if (gameObjects.get(i) instanceof Tank) {
                    if (((Tank) gameObjects.get(i)).getTankName().equals("Player two")) {
                        player1Won = true;
                    } else {
                        player2Won = true;
                    }
    ...
    private void checkCollisions() {
        for (int i = 0; i < gameObjects.size(); i++) {
            for (int j = 0; j < gameObjects.size(); j++) {
                ...
                if (((src.gameObjects.GameObject) co1).getRec().getBounds().intersects(((src.gameObjects.GameObject) co2).getRec().getBounds())) {
                    co1.collision(co2);
                    co2.collision(co1);
                }
            }
    ...
    public void paintComponent(Graphics g) {
        ...
        g2.drawString("Player one: ", 10, 28);
        g2.drawString("Player two: ", SCREEN_WIDTH / 2 + 10, 28);
        ...
        if (player1Won || player2Won) {
            ...
            if (player1Won) {
                g2.drawString("PLAYER ONE WINS! ", SCREEN_WIDTH / 4, SCREEN_HEIGHT / 2);
            }
            if (player2Won) {
                g2.drawString("PLAYER TWO WINS! ", SCREEN_WIDTH / 4, SCREEN_HEIGHT / 2);
            }
        }
